...
<div id="principal">
  ...
  <!-- InstanceBeginEditable name="conteudo" -->
    <p class="texto"><a href="default.asp">Início</a> &nbsp;&gt;&nbsp; Alunos</p>
    <p class="titulo">Alunos do 1º ano A</p>
    <p class="texto">Lista dos alunos do 1º ano A:</p>
    <table width="450" border="1" align="center" class="tabela">
      <tr>
        <td width="40">N°</td>
        <td width="10"><img src="imagens/icone_orkut.jpg" alt="Orkut" width="10" height="10" border="0" /></td>
        <td>Nome</td>
        <td width="90">Aniversário</td>
      </tr>
    <%
	...
	%>
      <tr>
		...
		%><%=bd("nome")%>&nbsp;<%=bd("sobrenome")%><%
		else
		%><a href="usuario.asp?id=<%=bd("id")%>"><%=bd("nome")%>&nbsp;<%=bd("sobrenome")%></a><%
		end if
		%></div></td>
        <td width="90" class="texto"><div align="left"><%if(len(bd("dianasc")) = 1) then Response.Write("0" & bd("dianasc")) else Response.Write(bd("dianasc")) end if%>/<%=mesextenso(bd("mesnasc"))%></div></td>
      </tr>
	<%
	bd.MoveNext
	wend
