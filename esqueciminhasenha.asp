...
<div id="principal">
  ...
  <!-- InstanceBeginEditable name="conteudo" -->
	...
	email = Replace(email, "'", "")
	if email <> Empty then
		bdemail = "SELECT * FROM cadastro WHERE email ='" & email & "'"
		bd.open bdemail, conexao
		if bd.EOF then
		naoencontrado = "sim"
		else
		naoencontrado = "nao"
	...
	%>
    <p class="texto"><a href="default.asp">Início</a> &nbsp;&gt;&nbsp; <a href="login.asp">Login</a> &nbsp;&gt;&nbsp; Esqueci minha senha</p>
    <p class="titulo">Esqueci minha senha</p>
    <p class="texto">Caso você tenha esquecido a sua senha, digite seu e-mail cadastrado no site para que o seu lembrete de senha apareça.</p>
    <form id="form1" name="form1" method="post" action="esqueciminhasenha.asp" onsubmit="return validaesqueceusenha('email');">
    <table width="450" border="0" align="center" class="texto">
      <tr>
        <td width="75"><div align="right">E-mail:</div></td>
        <td><label>
          <input name="email" type="text" class="texton" id="email" size="50" maxlength="50" onkeyup="validaesqueceusenha('email');" />
        </label></td>
      </tr>
      ...
      <tr id="infoemailvalido" style="display: none">
        <td></td>
        <td><label class="texto"><strong>digite um email válido</strong></label></td>
      </tr>
      <script>
	  emailerrado = "nao";
	  </script>
      <tr id="emailnaoencontrado" style="display: <%if naoencontrado = "sim" then%>block<%else%>none<%end if%>">
        <td></td>
        <td><label class="texto"><strong>email não encontrado em nosso banco de dados</strong></label></td>
      </tr>
      <tr>
        <td><div align="right"></div></td>
        <td><label>
          <input name="botao" type="submit" class="texton" id="botao" value="Lembrete" />
        </label></td>
      </tr>
    </table>
    </form>
    ...
    <%
	if naoencontrado = "nao" then
		%><p class="texto">Seu lembrete de senha é: <%=bd("lembretesenha")%></p><%
	end if
	%>
  <!-- InstanceEndEditable -->
  </div>
