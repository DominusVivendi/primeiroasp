...
<div id="principal">
  ...
  %>
	<%
	dim msginicial
	msginicial = "Deixe um comentário aqui!"
	if(request.form("add") = "Adicionar" AND request.form("conteudoa") <> "" AND request.form("conteudoa") <> msginicial) then
	adicionarcomentario
	end if
	...
	%>
    <script>
	function verificacampo(acao)
	{
		if(acao == "focus" && document.getElementById('conteudoa').value == "<%=msginicial%>")
		{
			document.comentarios.conteudoa.value = "";
		}
		if(acao == "blur" && document.getElementById('conteudoa').value == "")
		{
			document.comentarios.conteudoa.value = "<%=msginicial%>";
		}
	}
	</script>
    <p class="texto"><a href="default.asp">Início</a> &nbsp;&gt;&nbsp; Comentários</p>
    <form id="comentarios" name="comentarios" method="post" action="comentarios.asp">
    <p>
      <label>
      <textarea name="conteudoa" id="conteudoa" cols="50" rows="5" class="texto" onfocus="verificacampo('focus');" onblur="verificacampo('blur');"><%=conteudo%></textarea>
      </label>
      <br />
      <label>
	...
	if (request.form("visualizar") = "Visualizar") then
	%>
	<p class="texto"><strong>Comentário:</strong> <%
	if Request.Form("conteudoa") <> msginicial then
	%><%=Request.Form("conteudoa")%><%
	end if
	%></p>
	<%
	end if
	...
	<%
	bdid = "SELECT * FROM comentarios LEFT JOIN cadastro ON comentarios.idusuarioadd = cadastro.id ORDER BY comentarios.id DESC"
	bd.open bdid, conexao
	contador = 0
	while not bd.EOF
    %>
      <tr>
        <td width="75" valign="top"><hr /><strong><%
		if IsNull(bd("cadastro.id")) then
		%>ñ existe<%
		else
		%><a href="usuario.asp?id=<%=bd("cadastro.id")%>"><%=bd("nome")%></a><%end if%> diz:</strong></td>
        <td valign="top"><hr /><%=bd("conteudo")%></td>
        <td width="90" valign="top"><hr /><div align="right">
        <%
			if(day(bd("dataadd")) = day(date)) then
				...
				%>/<%=monthname(month(bd("dataadd")), true)%><%
			end if
			if(session("idusuario") = bd("idusuarioadd") OR session("tipo") = "admin") then
			%> - <strong><a href="javascript:void(0);" onclick="excluir('comentarios','<%=bd("comentarios.id")%>');">Excluir</a></strong><%
			end if
			%></div></td>
      </tr>
    <%
    ...
    <p class="texto"><strong>Existem <%=contador%> comentários!</strong></p>
    <%
	Sub adicionarcomentario()
	
	bd.Open "comentarios",conexao,3,3
	bd.AddNew
